...
def _send_test_email_sync(email_id, test_email, variables=None):
    ...
    # Add sender_email to variables
    if variables is None:
        variables = {}
    variables['sender_email'] = email.campaign.user.email
    ...
    # Create EmailSendRequest for test email to track it
    try:
        subscriber = Subscriber.objects.filter(email=test_email).first()
    except Exception:
        subscriber = None
    
    email_send_request = EmailSendRequest.objects.create(
    ...
    logger.info(f"Created EmailEvent {tracking_id} for test email {email.id}, campaign {email.campaign.id}, subscriber {test_email}")
    ...
    html_content = _replace_hr_with_separator(html_content, tracking_id, test_email, base_url=site_url)
    html_content = _wrap_links_with_tracking(html_content, tracking_id, test_email, base_url=site_url)
    ...
    try:
        ...
        # Update EmailSendRequest status to sent
        email_send_request.status = 'sent'
        email_send_request.sent_at = timezone.now()
        email_send_request.save(update_fields=['status', 'sent_at', 'updated_at'])
        ...
        # Update EmailSendRequest status to failed
        email_send_request.status = 'failed'
        email_send_request.error_message = str(e)
        email_send_request.save(update_fields=['status', 'error_message', 'updated_at'])
...
def _send_single_email_sync(email_id, subscriber_email, variables=None, request_id=None):
    ...
    # Get user email early to add to variables
    if request_obj and request_obj.user:
        sender_user_email = request_obj.user.email
    else:
        sender_user_email = email.campaign.user.email
    
    # Add sender_email to variables
    if variables is None:
        variables = {}
    variables['sender_email'] = sender_user_email

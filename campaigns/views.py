...
def email_detail(request, campaign_id, email_id):
    ...
    if request.method == 'GET':
        ...
        serializer = EmailSerializer(email, data=request.data, partial=True, context={'request': request})
        if serializer.is_valid():
...
def send_email(request, campaign_id, email_id):
    ...
    variables = request.data.get('variables', {})
    send_now = request.data.get('send_now', False)  # Flag to send immediately
    ...
    # Force immediate sending if send_now flag is set or wait_time is 0
    send_immediately = send_now or (wait_time == 0)
    
    send_delay = timedelta(0) if send_immediately else timedelta(0)
    ...
    # If scheduled for now or in the past, or if send_immediately flag is set, send immediately
    if send_immediately or scheduled_for <= timezone.now():
        try:
...
def campaign_edit(request, campaign_id):
    ...
    sent_paginator = Paginator(sent_emails, 10)  # 10 sent emails per page
    sent_emails_page = sent_paginator.get_page(sent_page)
    
    # Pagination for pending emails
    pending_page = request.GET.get('pending_page', 1)
    pending_paginator = Paginator(pending_emails, 10)  # 10 pending emails per page
    pending_emails_page = pending_paginator.get_page(pending_page)
    
    return render(request, 'campaigns/edit.html', {
        'campaign': campaign,
        'emails': emails,
        'lists': lists,
        'sent_emails': sent_emails_page,
        'pending_emails': pending_emails_page,
    })
...
def generate_email_with_ai(request, campaign_id):
    ...
    try:
        ...
        return Response({
            'error': str(e),
            'hint': 'Please ensure Ollama is running (ollama serve) and the llama3.2:1b model is available (ollama pull llama3.2:1b)'
        }, status=400)
    except Exception as e:
        import traceback
